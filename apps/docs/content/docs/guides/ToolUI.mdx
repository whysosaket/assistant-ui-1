...
Tools are functions that allow you to extend the capabilities of Large Language Models (LLMs) by implementing custom code that can be invoked during a conversation. They enable LLMs to perform specific tasks like searching the web, accessing databases, or interacting with external APIs. 

With assistant-ui, you can show a custom UI when a tool is called to let the user know what is happening. In this sample, we'll illustrate a tool that shows a weather UI when the user asks about the weather in Tokyo. The tool UI displays a loading state while fetching data, then shows the current temperature with appropriate day/night icons. 

<ToolUISample />

### Frontend & Backend Example With Vercel AI SDK

In this example, we'll be experimenting with a tool call for web search. This demonstrates how you can create interactive, visually appealing tool UIs that enhance the user experience during tool execution.

#### Frontend

```tsx title="/components/client-side-chat-component.tsx
...
```tsx title="/api/api/chat/route.ts
